...
//  if (scErrorNo) return;
	...
	va_start(arg, er);
	{
		...
		{
			sprintf(scErrorMsg, "%s,\n%s(%3d) Error", scErrorMsg, ST->name,
					ST->act_line + 1 + ST->linedif);
		} else
			sprintf(scErrorMsg, "%s(%3d) Error", ST->name,
					ST->act_line + 1 + ST->linedif);
		if (linenum)
			sprintf(scErrorMsg, "%s[%s:%d] :", scErrorMsg, filename,
					linenum);
		else
		...
		if (debug_flag) {
			fprintf(debfile, "\nScript(%3d) Error[%s:%d]: ",
					ST->act_line + 1, filename, linenum);
	...
	va_start(arg, er);
	{
		...
		if (scActual_Instance) {
			if (scActual_Instance->forked)
				sprintf(scErrorMsg, "%s in `%s'(%d):", scErrorMsg,
						scGet_Instance_Title(scActual_Instance),
						scActual_Instance->flags.forkno);
			else
				sprintf(scErrorMsg, "%s in `%s':", scErrorMsg,
						scGet_Instance_Title(scActual_Instance));
		} else
...
bool scget_lexema(Source_Text * ST)
{
	...
	if (!parser_Comment && strcmp(ST->lexema, "\"") == 0) {	//Read all string
		char temp[10000];
		temp[i++] = '\"';
		do {
		READ_INSIDE_STRING} while (c != '\"');
		temp[i] = 0;
		strcpy(ST->lexema, temp);
	} else if (!parser_Comment && strcmp(ST->lexema, "'") == 0) {	//Read all string
		char temp[10000];
		temp[i++] = '\'';
		do {
		READ_INSIDE_STRING} while (c != '\'');
