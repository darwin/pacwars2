...
char *scReadOperatorName()
{
	...
	if (lexeq("==") || lexeq("<=") || lexeq(">=") || lexeq("+=")
		|| lexeq("-=") || lexeq("/=") || lexeq("*=") || lexeq("%=")
		|| lexeq("++") || lexeq("--") || lexeq("!=") || lexeq("<<")
		|| lexeq(">>") || lexeq("[]") || lexeq("^=") || lexeq("&=")
		|| lexeq("|=")) {
		name = scStrdup("operator??");
			...
			if lexeq
				...
				if (!ts && actu)
					serr2(scErr_Declaration,
						  "Parameter declaration expected at %s",
						  ST->lexema);
				if (!ts) {
					scSetPosition(sp);
					Advance;
				...
				if (ts->name)	//check if duplicated names
					for (it = 0; it < actu - 1; it++)
						if (sm[it]->name
							&& strcmp(sm[it]->name, ts->name) == 0)
							serr4(scErr_Declaration,
								  "Parameters %d and %d has the same name '%s'",
								  it + 1, actu, ts->name);
//     deb1("<%x>\n",(int)ts);
			...
			Advance;
			if (lexeq("=")) {	//serr( scErr_Parse,"SeeR now doesn't allow default initializers in function parameters")
				int ip;
				char s[12];
				...
				while (lexeq("[")) {
							 ...
							 "[] is illegal in SeeR. Sorry :-(");
					{
						...
						if (!scvalIsImmidiateOfType(&val, valINT)
							|| (act.ip != ip))
							serr2(scErr_Declaration,
								  "Constant integer array size expected",
								  ST->lexema);
						i = val.adr.val.ival;
						if (!lexeq("]"))
							serr2(scErr_Declaration,
				...
				if (!
					(structure =
					 scdicFoundSymbol(SC_types->first,
									  *name))) serr2(scErr_Declaration,
													 "Structure `%s' does not exist",
													 *name);
				if (
					(sctypGetValType(&structure->type) != valSTRUCT
					 && sctypGetValType(&structure->type) != valSCRIPT)
					|| !(structure->adr.flags & adrTYPEONLY))
					serr2(scErr_Declaration, "`%s' is not a structure",
						  *name);
				while (lexeq(".") || lexeq("::")) {
					scSymbol *newsym;
					if (!*name)
					...
					if (!structure->type.main)	//this was a fully defined structure
						serr2(scErr_Declaration,
							  "Structure `%s' was not fully specified",
							  *name);
					Advance;
					if (lexeq("~")) {
						int i;
					...
					if (!
						(newsym =
						 scdicFoundSymbol(structure->type.main,
										  ST->
										  lexema)))
				   serr3(scErr_Declaration, "`%s' is not a member of %s",
						 ST->lexema, structure->name);
					{			//add to name
						char *x =
							scAlloc(strlen(*name) + strlen(ST->lexema) +
									3);
						strcpy(x, *name);
						strcpy(x + strlen(x), ".");
					...
					structure =
						(((newsym->
						   type.flags & typTYPE) == typSTRUCT
						  || (newsym->type.flags & typTYPE) == typSCRIPT)
						 && (newsym->adr.
							 flags & adrTYPEONLY)) ? newsym : NULL;
					Advance;
				}				//while
				scget_back(ST);
			}					//if
...
//    serr(scErr_Declaration,"script cannot be a member of any function or structure");
		...
		if (!lexeq("{"))		//name:
		{
			if (!sciValidIdentifier(ST->lexema))
				serr2(scErr_Declaration,
					  "Invalid identifier `%s' in script declaration",
					  ST->lexema);
			if (
				(structure =
				 scdicFoundSymbol(SC_types->first, ST->lexema))) {
				if (sctypGetValType(&structure->type) != valSCRIPT)
					serr2(scErr_Declaration,
						  "`%s' already defined as non-script",
						  ST->lexema);
				if (structure->type.main)	//this was a fully defined structure
					serr2(scErr_Declaration,
						  "duplicate definition of `%s'", ST->lexema);
			} else {
			...
			serr2(scErr_Declaration,
				  "Identifier required in script declaration", ST->lexema);
...
//      serr( scErr_Declaration,"script cannot be a member of any structure");
			if (!sym2->name) {
				scSymbol *x;
				if (sctypGetValType(&sym2->type) != valSTRUCT
					&& sctypGetValType(&sym2->type) != valSCRIPT)
					serr(scErr_Declaration,
						 "member name not specified in struct declaration");
				scAssert(scsGetTypeDeli,
						 sym2->type.flags == typUSERDEFINED);
				x = sym2;
...
//       scFree(x);
			...
			if (sctypGetValType(&sym2->type) == valSTRUCT
				|| sctypGetValType(&sym2->type) == valSCRIPT) {
				if (sym2->type.flags == typUSERDEFINED
					&& sym2->type.main == structure && (!sym2->type.params
														|| !sym2->type.
														params[0]))
					serr2(scErr_Declaration,
						  "recursive declaration (script '%s' inside itself)",
						  structure->name);
				if (sym2->type.flags == typUSERDEFINED
					&& !sym2->type.main->type.main && (!sym2->type.params
													   || !sym2->type.
													   params[0]))
					serr2(scErr_Declaration,
						  "struct '%s' not fully specified",
						  sym2->type.main->name);
			}
			...
			if (scdicFoundSymbol(sym->type.main, sym2->name))
				serr2(scErr_Declaration, "member `%s' already in script",
					  sym2->name);
			...
			if (lexeq(",") && sctypIsFunction(&sym1->type))
				serr(scErr_Declaration,
					 "function can be declared one at a time!");
			while (lexeq(",")) {
				Advance;
				scget_back(ST);
				if (!(sym2 = sctypGetShortDefinition(&sym1->type)))
					serr(scErr_Declaration,
						 "not a type in type definition");
...
//      serr( scErr_Declaration,"script cannot be a member of any structure");
			...
			if (lexeq("{") && sctypIsFunction(&sym2->type)) {
				serr2(scErr_Declaration,
					  "What? You want to define function `%s'? It'll come in the script!",
					  sym2->name);
			} else if (!lexeq(";"))
				serr2(scErr_Parse, parse_error, ST->lexema);
			Advance;
	...
	{							//struct
		...
		if (!lexeq("{"))		//name:
		{
			if (!sciValidIdentifier(ST->lexema))
				serr2(scErr_Declaration,
					  "Invalid identifier `%s' in struct declaration",
					  ST->lexema);
			//WORK:
			if (
				(!this_symbol.type.main
				 && (structure =
					 scdicFoundSymbol(SC_types->first, ST->lexema)))
				|| (this_symbol.type.main
					&& (structure =
				...
				if (sctypGetValType(&structure->type) != valSTRUCT)
					serr2(scErr_Declaration,
						  "`%s' already defined as non-structure",
						  ST->lexema);
				if (structure->type.main)	//this was a fully defined structure
					serr2(scErr_Declaration,
						  "duplicate definition of `%s'", ST->lexema);
...
//      structure->add=type;
			...
			serr2(scErr_Declaration,
				  "Identifier required in struct declaration", ST->lexema);
...
//      else
			...
			if (sctypGetValType(&sym2->type) == valSTRUCT
				...
				if (sym2->type.flags == typUSERDEFINED
					&& sym2->type.main == structure && (!sym2->type.params
														...
														params[0]))
					serr2(scErr_Declaration,
						  "recursive declaration (struct '%s' inside itself)",
						  structure->name);
				if (sym2->type.flags == typUSERDEFINED
					&& !sym2->type.main->type.main && (!sym2->type.params
													   || !sym2->type.
			...
			if (sym2->name && sym2->name[0] == '~'
				&& strcmp(sym2->name + 1, structure->name) == 0) {
				...
				if (sctypIsFunction(&sym2->type)) {
					while (p) {
						if (strcmp(sym2->name, p->name) == 0 &&
							(!sctypIsFunction(&p->type)
							 || sctypExactFunction(&p->type, &sym2->type)))
							serr2(scErr_Declaration,
								  "member `%s' already in structure",
								  sym2->name);
						p = p->next;
					}
				} else
					serr2(scErr_Declaration,
						  "member `%s' already in structure", sym2->name);
			}


			if (!sctypIsFunction(&sym2->type)
				&& !(sym2->adr.flags & adrTYPEONLY)) {
				if (imported)
					serr2(scErr_Declaration,
						  "A member variable (%s) cannot be imported",
						  sym2->name);
			...
			if (lexeq("{") && sctypIsFunction(&sym2->type)) {
				...
				if (imported)
					serr2(scErr_Declaration,
						  "What? You want to define function `%s'? It's already imported!",
						  sym2->name);
#if DEBUGLEVEL==0
			...
			if (imported)		//has to be function
			{
				if (!sctypIsFunction(&sym2->type))
					serr(scErr_Declaration,
						 "Member variables cannot be imported.");
		...
		while (i) {
			...
			if (lexeq("unsigned")) {
				...
				if (special_was_signed)
					serr(scErr_Declaration,
						 "Declare both signed and unsigned?");
			}
			if (lexeq("signed")) {
				Advance;
				i = 1;
...
//Store the position of starting of the real type or name if constructor/destr
		...
		for (i = 0; Standard_Types[i].name; i++)
			if (lexeq(Standard_Types[i].name)) {
				...
				{
					serr(scErr_Declaration,
						 "Type long is a 64bit long, yet not supported. \n(if you need 32-bit long, use int instead)");
				}
				...
				if (sym->type.flags & typUNSIGNED) {
					...
					default:
						serr(scErr_Declaration,
							 "Only char,short and int can be unsigned");
					}
				}
				return sym;
			}
		...
		if (this_symbol.reserved)	//we're in a struct
		{
			...
			if (
				...
				if (sym1->adr.flags & adrTYPEONLY) {
#if DEBUGLEVEL==0
					deb2("Declaration:user-defined `%s.%s' ",
						 this_symbol.type.main->name, ST->lexema);
#endif
					sym->type.flags |= typUSERDEFINED;
					sym->type.main = sym1;
					Advance;
