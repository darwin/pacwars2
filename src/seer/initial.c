	...
	case valSTRUCT:
		if (lexeq("{")) {
			...
			while (p) {
				...
				j =
					scsReadDirectInitialization(deli, &p->type, true, dest,
												inner_size, NULL);
				...
				if (p && !lexeq(","))
					serr2(scErr_Declaration,
						  "`,' expected instead of `%s'", ST->lexema);
			}
			if (lexeq(",")) {
				Advance;
			}
	...
	default:
		{
			...
			if (deli)			//STACK
			{
				...
				if (type_size == 4
					&& (v == valINT || v == valPOINTER || v == valFIXED
						|| v == valFLOAT)) {
					...
					Gen_Opcode(opvcpuMOV,
							   scvalSetRegister(&val1, valINT, regCX),
							   scvalSetINT(&val2, type_size));
					scvalArrayToPointer(&val);	//maybe needed
					scvalSetINT(&val1, init_pos);
					val1.adr.flags = adrBP;
					val1.adr.address = init_pos;
...
{								//_value val;
	...
	if (sctypIsFunction(&type))
		serr2(scErr_Declaration, "Declaring function `%s' with `='",
			  sym->name);
	Advance;
...
//  v.type=sym->type;sctypGoDeeper(&v.type);
		...
		if (scvalGetValType(&val1) != sctypGetValType(&sym->type))
			serr2(scErr_Declaration,
				  "types do not match for initialization of reference `%s'",
				  sym->name);
		...
		if (deli)				//stack
		{
			...
			{
				serr2(scErr_Declaration,
					  "invalid initialization of reference `%s'",
					  sym->name);
			}
			//TODO:initialization code for heap version
			serr2(scErr_Declaration,
				  "invalid initialization of reference `%s'", sym->name);
		}
		return;
	}
	deb0("\n");
...
//1      serr2(scErr_Declaration,"invalid initializer for static `%s'",sym->name);
		...
		fsym =
			scsFind_Function(_this->name, _this->type.main, NULL, params,
							 NULL);
		if (deli) {				//local
			if (fsym) {
				Gen_Opcode(opvcpuSUB,
						   scvalSetRegister(&val1, valINT, regSP),
						   scvalSetINT(&val2, type_size));
				...
				if (!scsTypesExact(&sym->type, &params[0].type))
					serr2(scErr_Declaration,
						  "invalid initializer for `%s'", sym->name);
				if (type_size == 4)
					...
					Gen_Opcode(opvcpuSUB,
							   scvalSetRegister(&val1, valINT, regSP),
							   scvalSetRegister(&val2, valINT, regCX));
...
/****************STACK**********************/
	if (deli) {
		...
		if (
			(size =
			 scsReadDirectInitialization(deli, &type, false, &act.consts,
										 type_size, sym))) {
			char *code = NULL;
			_value val3;
			...
			{
				deb2
					("Code (%d to %d) was generated in initialization.It will be moved after the copy.\n",
					 ip, act.ip - 1);
				ip = act.ip - ip;
				act.ip -= ip;
				code = scAlloc(ip);
			...
			/*SUB SP,CX;COPY */
			Gen_Opcode(opvcpuMOV, scvalSetRegister(&val1, valINT, regCX),
					   scvalSetINT(&val2, size));
		...
		if (
			(size =
			 scsReadDirectInitialization(deli, &type, true, &act.inits,
										 type_size, sym))) {
