...
int callfunc(void *func, void *params, int count)
{
	...
	if (!count) {
		__asm {
	call func mov reti, eax}} else {
		__asm {
			sub esp, count
				mov esi, params
				mov ecx, count shr ecx, 2 jcxz endl1} loop1:__asm {
			mov eax,[esi]
				mov[esp], eax
				add esp, 4
				add esi, 4 loop loop1 sub esp, count} endl1:__asm {
	call func
				mov reti, eax
				mov[_msvc_regEDX], edx add esp, count}} return reti;
}
...
int callmember(void *func, void *params, int count)
{
	...
	call func mov reti, eax}} else {
		__asm {
			sub esp, count
				mov esi, params mov ecx, count shr ecx, 2 mov edx,[esi]	//copy 'this' to edx
		add esi, 4 dec ecx sub count, 4 jcxz endl1m} loop1m:__asm {
			mov eax,[esi]
				mov[esp], eax
				add esp, 4
				add esi, 4 loop loop1m sub esp, count} endl1m:__asm {
...
//    ;
	if (!count) {				//pop edi,esi,ebx, fld laduje - fsv
		__asm {
			call func fst retd;
			pop ebx;
			pop esi;
		...
		__asm {
			mov esi, params
				sub esp, count
				mov ecx, count shr ecx, 2 jcxz endl2} loop2:__asm {
			mov eax,[esi]
				mov[esp], eax
				add esp, 4
				add esi, 4 loop loop2 sub esp, count} endl2:__asm {
			call func add esp, count fst retd;
			pop ebx;
			pop esi;
			pop edi;
			 leave;
	 ret}} return retd;			//ret was earlier, it's only to disable a warning
}
...
//    ;
	...
	{							//pop edi,esi,ebx, fld laduje - fsv
		__asm {
	call func fst retd}} else {
		__asm {
			mov esi, params
				sub esp, count mov ecx, count shr ecx, 2 mov edx,[esi]	//copy 'this' to edx
		add esi, 4 dec ecx sub count, 4 jcxz endl2m} loop2m:__asm {
			mov eax,[esi]
				mov[esp], eax
				add esp, 4
				add esi, 4 loop loop2m sub esp, count} endl2m:__asm {
			mov ecx, edx		//'this' should be in ecx
	 call func add esp, count fst retd}} return retd;
}
	...
	if (!count) {
	  ...
	  :	"memory"			/* assume everything is clobbered */
		...
		asm("  subl %2,%%esp ; "
			...
			"  loop loop1 ; "
			"  subl %2,%%esp ; "
	  "  endl1: " "  call *%%edi ; " "  addl %2,%%esp  ":"=&a"(ret)
								/* return value in eax */
	  :	"D"(func),			/* function in edi */
...
int callfunc(void *func, void *params, int count)
{
	...
	case 9:
		return _INT_FUNC((int, int, int, int, int, int, int, int, int))
			...
			(_I_PA(0), _I_PA(1), _I_PA(2), _I_PA(3), _I_PA(4), _I_PA(5),
			 _I_PA(6), _I_PA(7), _I_PA(8));
	case 10:
		return
			_INT_FUNC((int, int, int, int, int, int, int, int, int, int))
			(_I_PA(0), _I_PA(1), _I_PA(2), _I_PA(3), _I_PA(4), _I_PA(5),
			 _I_PA(6), _I_PA(7), _I_PA(8), _I_PA(9));
	case 11:
		return
			_INT_FUNC(
					  (int, int, int, int, int, int, int, int, int, int,
					   int)) (_I_PA(0), _I_PA(1), _I_PA(2), _I_PA(3),
							  _I_PA(4), _I_PA(5), _I_PA(6), _I_PA(7),
							  _I_PA(8), _I_PA(9), _I_PA(10));
	case 12:
		return
			_INT_FUNC(
					  (int, int, int, int, int, int, int, int, int, int,
					   int, int)) (_I_PA(0), _I_PA(1), _I_PA(2), _I_PA(3),
								   _I_PA(4), _I_PA(5), _I_PA(6), _I_PA(7),
								   _I_PA(8), _I_PA(9), _I_PA(10),
								   _I_PA(11));
	default:
...
double callfunc_d(void *func, void *params, int count)
{
	...
	case 10:
		return
			_DBL_FUNC((int, int, int, int, int, int, int, int, int, int))
			(_I_PA(0), _I_PA(1), _I_PA(2), _I_PA(3), _I_PA(4), _I_PA(5),
			 _I_PA(6), _I_PA(7), _I_PA(8), _I_PA(9));
	case 11:
		return
