...
{
  int i, j;
  unsigned int k, rshift, gshift, bshift, rbits, gbits, bbits, rval, gval, bval;
  ...
  mpf = mapTiles[0]->format;
	if (mpf->BitsPerPixel>8) {
		j = (int) mpf->Rmask; rshift = 0;
		i = 1; while (!(i&j)) { rshift++; i<<=1; }
		rbits = 0; while (i&j) { rbits++; i<<=1; }
		j = (int) mpf->Gmask; gshift = 0;
		i = 1; while (!(i&j)) { gshift++; i<<=1; }
		gbits = 0; while (i&j) { gbits++; i<<=1; }
		j = (int) mpf->Bmask; bshift = 0;
		i = 1; while (!(i&j)) { bshift++; i<<=1; }
		bbits = 0; while (i&j) { bbits++; i<<=1; }
	}
	...
	{
		mss = mapTiles[i];
    if ((SDL_MUSTLOCK(mss)) && (SDL_LockSurface(mss)!=0)) return -1;
		...
		rval <<= rshift; rval &= mpf->Rmask;
		gval <<= gshift; gval &= mpf->Gmask;
		bval <<= bshift; bval &= mpf->Bmask;
...
int MapRelocate(CMapState* ms)
{
  ...
  i = mapnumblockstr; j = mapblockwidth*mapblockheight*((mapdepth+1)/8);
    ...
    myblkstrpt->bgoff = (((long int) myblkstrpt->bgoff)/j);
		if (myblkstrpt->fgoff!=0) myblkstrpt->fgoff = (((long int) myblkstrpt->fgoff)/j);
		if (myblkstrpt->fgoff2!=0) myblkstrpt->fgoff2 = (((long int) myblkstrpt->fgoff2)/j);
    if (myblkstrpt->fgoff3!=0) myblkstrpt->fgoff3 = (((long int) myblkstrpt->fgoff3)/j);
		myblkstrpt++; i--;
	}
...
int MapDecodeBKDT (CMapState* ms)
{
  ...
  ms->mapblockstrpt = (char *) malloc (ms->sblockstr);
	if (ms->mapblockstrpt==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeANDT (CMapState* ms)
{
  ...
  ms->mapanimstrpt = (char *) malloc (ms->sanimstr);
	if (ms->mapanimstrpt==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeBODY (CMapState* ms)
{
  ...
  ms->mapmappt[0] = (short int *) malloc (ms->smapmap[0]);
	if (ms->mapmappt[0]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR1 (CMapState* ms)
{
  ...
  ms->mapmappt[1] = (short int *) malloc (ms->smapmap[1]);
	if (ms->mapmappt[1]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR2 (CMapState* ms)
{
  ...
  ms->mapmappt[2] = (short int *) malloc (ms->smapmap[2]);
	if (ms->mapmappt[2]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR3 (CMapState* ms)
{
  ...
  ms->mapmappt[3] = (short int *) malloc (ms->smapmap[3]);
	if (ms->mapmappt[3]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR4 (CMapState* ms)
{
  ...
  ms->mapmappt[4] = (short int *) malloc (ms->smapmap[4]);
	if (ms->mapmappt[4]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR5 (CMapState* ms)
{
  ...
  ms->mapmappt[5] = (short int *) malloc (ms->smapmap[5]);
	if (ms->mapmappt[5]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR6 (CMapState* ms)
{
  ...
  ms->mapmappt[6] = (short int *) malloc (ms->smapmap[6]);
	if (ms->mapmappt[6]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapDecodeLYR7 (CMapState* ms)
{
  ...
  ms->mapmappt[7] = (short int *) malloc (ms->smapmap[7]);
	if (ms->mapmappt[7]==NULL) { maperror = MER_OUTOFMEM; return -1; }
...
int MapLoad (char * name, char *script)
{
...

  {
    ...
    if (map_script_num!=-1) ScriptMan->DeleteScript(map_script_num); // unload old script
    map_script_num = ScriptMan->NewScript(script, smap_ext.string);
    if (map_script_num==-1) return -10;
    if (ScriptMan->InstanceScript(map_script_num)!=0) return -10;
  ...
  mapfilept = fopen (mapname, "rb");
	...
	if (mapfilesize!=((Mapbyteswapl(mapgenheader.headsize))+8)) maperror = MER_MAPLOADERROR;
  ...
  maperror=MER_MAPLOADERROR;
	...
	while (mapfilesize!=((unsigned long)ftell(mapfilept)))
	{
		...
		{ if (mapgenheader.id4=='D') { MapDecodeMPHD (ms); i = 1; } } } }
		if (mapgenheader.id1=='C') { if (mapgenheader.id2=='M') { if (mapgenheader.id3=='A')
    { if (mapgenheader.id4=='P') { MapDecodeCMAP (ms); i = 1; } } } }
		if (mapgenheader.id1=='B') { if (mapgenheader.id2=='K') { if (mapgenheader.id3=='D')
		{ if (mapgenheader.id4=='T') { MapDecodeBKDT (ms); i = 1; } } } }
		if (mapgenheader.id1=='A') { if (mapgenheader.id2=='N') { if (mapgenheader.id3=='D')
		{ if (mapgenheader.id4=='T') { MapDecodeANDT (ms); i = 1; } } } }
		if (mapgenheader.id1=='B') { if (mapgenheader.id2=='G') { if (mapgenheader.id3=='F')
		{ if (mapgenheader.id4=='X') { MapDecodeBGFX (ms); i = 1; } } } }
		if (mapgenheader.id1=='B') { if (mapgenheader.id2=='O') { if (mapgenheader.id3=='D')
		{ if (mapgenheader.id4=='Y') { MapDecodeBODY (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='1') { MapDecodeLYR1 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='2') { MapDecodeLYR2 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='3') { MapDecodeLYR3 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='4') { MapDecodeLYR4 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='5') { MapDecodeLYR5 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='6') { MapDecodeLYR6 (ms); i = 1; } } } }
		if (mapgenheader.id1=='L') { if (mapgenheader.id2=='Y') { if (mapgenheader.id3=='R')
		{ if (mapgenheader.id4=='7') { MapDecodeLYR7 (ms); i = 1; } } } }
		if (!i) MapDecodeNULL (ms);
		if (maperror) 
    { 
      fclose (mapfilept); 
      return -1; 
...
CMapState::CMapState()
{
  int i;
	...
	for (i=0;i<8;i++) mapmappt[i]=NULL;
	mappt = NULL;
	for (i=0;i<8;i++) mapmaparraypt[i]=NULL;
	maparraypt = NULL;
	mapblockstrpt = NULL;
	mapanimstrpt = NULL;
...
int CMapState::Copy(CMapState* o)
{
	if (o->state==MS_INITED)
  {
    ...
    mapblockstrpt = (char *) malloc (sblockstr);
	  if (mapblockstrpt==NULL) return MER_OUTOFMEM;
    memcpy(mapblockstrpt, o->mapblockstrpt, sblockstr);

    sanimstr = o->sanimstr;
    mapanimstrpt = (char *) malloc (sanimstr);
	  if (mapanimstrpt==NULL) return MER_OUTOFMEM;
	  ...
	  for (int i=0; i<8; i++)
    {
      ...
      if (smapmap[i])
      {
        mapmappt[i] = (short int *) malloc (smapmap[i]);
	      if (mapmappt[i]==NULL) return MER_OUTOFMEM;
...
BLKSTR * CMapState::GetBlock (int x, int y)
{
  ...
  if (x<0) x+=mapwidth; else if (x>=mapwidth) x-=mapwidth;
  if (y<0) y+=mapheight; else if (y>=mapheight) y-=mapheight;
...
/*	for (i=0;i<mapnumblockgfx;i++)
  ...
  // Draw top left clipped block
	...
	if (*mymappt>=0) blkdatapt = (BLKSTR*) (((char *)mapblockstrpt) + *mymappt);
	  ...
	  if ( SDL_BlitSurface(mapTiles[blkdatapt->bgoff], &srrec,  Dest, &dsrec) == -2 ) {
		  return 1;  
    }
  ...
  // Draw top left clipped block
    ...
    if (blkdatapt->fgoff) {
      if (blkdatapt->tl2)  // transparency bit
        SDL_SetColorKey(mapTiles[blkdatapt->fgoff], SDL_SRCCOLORKEY, transpcol);

      if ( SDL_BlitSurface(mapTiles[blkdatapt->fgoff], &srrec,  Dest, &dsrec) == -2 ) {
		    return 1;  
      }
