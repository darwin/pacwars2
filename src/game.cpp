...
CGame::~CGame()
{
  map.Free();
  MapFreeMem();
  for (int i=0; i<GAME_MAX_OBJS; i++) if (objs[i]) 
  {
    delete objs[i];
  }
...
void CGame::Clear()
{
  ...
  map.Free();
  for (i=0; i<GAME_MAX_OBJS; i++) if (objs[i]) 
  {
    delete objs[i];
    objs[i] = new GObj(0, this, i);
...
*/
  ...
  default:
    fprintf(stderr, "RebornObject: attempt to reborn to unknown object");
  }
...
GAME_MAXOBJS_TYPE CGame::FindUnusedSlot()
{
  ...
  {
    if (!(objs[i]->state&OSTATE_ACTIVE)) return i;
  }
...
void CGame::ServerThink(TICK_TYPE time)
{
  ...
  for (GAME_MAXOBJS_TYPE i=0; i<GAME_MAX_OBJS; i++) 
  {
    if (objs[i]->state&OSTATE_ACTIVE) objs[i]->ServerThink(time);
  }
}
...
void CGame::ClientThink(TICK_TYPE time)
{
  ...
  for (GAME_MAXOBJS_TYPE i=0; i<GAME_MAX_OBJS; i++) 
  {
    if (objs[i]->state&OSTATE_ACTIVE) objs[i]->ClientThink(time);
  }
}
      ...
      else
      {
        if (c>1000)
        {
          if (c/1000!=last)
          {
            ...
            {
              if (last==10 || last<=3) SV_BroadcastPrintf("Server: %d seconds remain", last);
              if (last==10) 
...
void CGame::UpdateGamebarSlots()
{
  ...
  for (i=0; i<GAME_MAX_OBJS; i++)
  {
    if ((objs[i]->GetType()==ot_player) && (objs[i]->state&OSTATE_ACTIVE))
    {
      ...
      if (p->brain_owner==client_info.client_num) MainSlot=i;
  ...
  if (slot!=MAX_GAMEBAR_PLAYERS) GBSlots[slot] = GAME_MAX_OBJS; // end marker
  ...
  while (!sorted)
  {
    ...
    if (GBSlots[0]!=GAME_MAX_OBJS)
      ...
      {
        if (GBSlots[i+1]==GAME_MAX_OBJS) break;
  ...
  if (last_leader!=GAME_MAX_OBJS && GBSlots[0]!=GAME_MAX_OBJS)
  {
    if (last_leader!=GBSlots[0])
    {
      ...
      if (objs[last_leader]->GetType()==ot_player && p1->brain_type==bt_client && p1->brain_owner==client_info.client_num) smPlayVoice(SM_LOSTLEAD, 100, 80);
      if (objs[GBSlots[0]]->GetType()==ot_player && p2->brain_type==bt_client && p2->brain_owner==client_info.client_num) smPlayVoice(SM_TAKENLEAD, 100, 80);
    }
  }
...
void CGame::RenderPlayerBar(GPlayer* p, SDL_Surface* screen, int font, int px, int py)
{
  SDL_Rect r;
  SDL_Rect s;
  ...
  char buf[40];
  p->player_name.GetValRef()->chars[p->player_name.GetValRef()->len-1]=0;
  DrawText(p->player_name.GetValRef()->chars, screen, font, px+42, py+3); 
...
void CGame::RenderGameBar(SDL_Surface* screen, int font, int px, int py)
{
  ...
  {
    if (GBSlots[i]==GAME_MAX_OBJS) break;
    if (objs[GBSlots[i]]->GetType()==ot_player && (objs[GBSlots[i]]->state&OSTATE_ACTIVE))
    {
...
#define SB_BLINK_TIME 60 // 2s with FPS=30
  ...
  if (MainSlot!=-1)
  {
    ...
    if (p->state&OSTATE_ACTIVE) 
    {
      ...
      {
        if (!l1) { w1 = tick; l1 = true; }
        s.x = 36;
        if (tick-w1<SB_BLINK_TIME && ((tick-w1)&4)) s.x = 0;
      ...
      {
        if (!l2) { w2 = tick; l2 = true; }
        s.x = 36;
        if (tick-w2<SB_BLINK_TIME && ((tick-w2)&4)) s.x = 0;
        r.x = px + 2 + 1*64;
        s.y = 1*25; 
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
      ...
      {
        if (!l3) { w3 = tick; l3 = true; }
        s.x = 36;
        if (tick-w3<SB_BLINK_TIME && ((tick-w3)&4)) s.x = 0;
        r.x = px + 2 + 2*64;
        s.y = 2*25; 
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
      ...
      {
        if (!l4) { w4 = tick; l4 = true; }
        s.x = 36;
        if (tick-w4<SB_BLINK_TIME && ((tick-w4)&4)) s.x = 0;
        r.x = px + 2 + 3*64;
        s.y = 3*25;
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
      ...
      {
        if (!l5) { w5 = tick; l5 = true; }
        s.x = 36;
        if (tick-w5<SB_BLINK_TIME && ((tick-w5)&4)) s.x = 0;
        r.x = px + 2 + 4*64;
        s.y = 4*25; 
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
      ...
      if (p->shields) 
      {
        if (h6!=p->shields) { w6 = tick; }
        s.x = 36;
        if (tick-w6<SB_BLINK_TIME && ((tick-w6)&4)) s.x = 0;
        r.x = px + 2 + 5*64;
        s.y = 5*25; 
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
      ...
      if (p->warps) 
      {
        if (h7!=p->warps) { w7 = tick; }
        s.x = 36;
        if (tick-w7<SB_BLINK_TIME && ((tick-w7)&4)) s.x = 0;
        r.x = px + 2 + 6*64;
        s.y = 6*25;
        SDL_BlitSurface(statusbar, &s, screen, &r);
      }
    ...
    if ((e->state&OSTATE_ACTIVE) && (e->state&OSTATE_VISIBLE))
    {
      if (MainSlot!=-1)
      {
        ...
        if (p->state&OSTATE_ACTIVE) 
        {
          ...
          if (ABS(dx)>mappxwidth/2) dx=(-SIGN(dx))*(mappxwidth-ABS(dx));
          if (ABS(dy)>mappxheight/2) dy=(-SIGN(dy))*(mappxheight-ABS(dy));
...
void CGame::RenderResults(SDL_Surface* screen, int font)
{
  ...
  for (i=0; i<MAX_GAMEBAR_PLAYERS; i++) 
  {
    ...
    if (objs[GBSlots[i]]->GetType()==ot_player)
    {
      ...
      sprintf(line, "%-10s +  %3d   ", p->player_name.GetValRef()->chars, (int)*p->frags.GetValRef());
      DrawText(line, screen, font, px, py); 
      py+=13;
    }
  }
