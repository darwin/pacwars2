...
// cvars
cvar_t net_inactive_timeout =
	{ "inactive_timeout", STD_INACTIVE_TIMEOUT, true, true };
...
Uint16 CRC(void *buffer, int length)
{
	...
	return (Uint16) (adler32(adler, (const unsigned char *) buffer, length)
					 & 0xFFFF);
}
...
int net_msg::Receive(net_message_pool * pool, net_stats * stats,
					 UDPsocket sock, int channel, UDPpacket * packet,
					 bool confirm)
{
...
int net_msg::Send(net_message_pool * pool, net_stats * stats,
				  UDPsocket sock, int channel, bool reliable,
				  Uint32 resend, net_msg_callback * msgcb, Uint16 tag)
{
	// stats
	stats->PacketsSent++;
	return SendMsg(pool, sock, channel, buf, pos, reliable, resend, msgcb,
				   tag);
}
...
int SendMsg(net_message_pool * pool, UDPsocket sock, int channel,
			void *data, int len, bool reliable, Uint32 resend,
			net_msg_callback * msgcb, Uint16 tag)
{
	int i;
	net_message *msg;
...
//    ((Uint8*)data)[rand()%len] = rand()%256;
	...
	if (!reliable)				// if this is not reliable packet, confirm automatically
	{
		...
		if (msg->msgcb)
			msg->msgcb(msg);	// call msg callback
...
int SendMsg(net_message_pool * pool, UDPsocket sock, int channel,
			net_msg msg, bool reliable, Uint32 resend,
			net_msg_callback * msgcb, Uint16 tag)
{
	return SendMsg(pool, sock, channel, msg.buf, msg.pos, reliable, resend,
				   msgcb, tag);
}
...
int ReceiveMsg(net_message_pool * pool, UDPsocket sock, int channel,
			   UDPpacket * packet, bool confirm)
{
...
int CreatePool(net_message_pool * pool, int send_size, int rec_size,
			   int packet_size)
{
	pool->msg_num = 1;
	pool->rsize = rec_size;
...
int LoadFileBuffer(z_streamp zs, Bytef * quantum, int qsize,
				   Bytef * buffer, int bsize, FILE * file,
				   int *file_status)
{
	int count;
	int err;
...
int StoreFileBuffer(z_streamp zs, Bytef * quantum, int qsize,
					Bytef * buffer, int bsize, FILE * file,
					int *file_status)
{
	zs->next_in = quantum;
