...
void CommandExecute(char *Input, ...)
{
  ...
  if (Command[0]!=REMOTE_CMD_CHAR)
  {
    ...
    { // wasn't cvar
      ...
      if (net_client_status!=NS_UNINITED) ChatMessage(BackStrings);
    }
  }
  else // remote command
  {
    buffer[POS_MSG_ID] = MSGID_REMOTECMD;
    int len = snprintf((char*)&buffer[POS_MSG_CMD], LEN_MSG_CMD, "%s", &BackStrings[1]);
    len+= POS_MSG_CMD + 1;
    SendMsg(&server.msg_pool, msg_csock, 0, buffer, len, true, RESEND_SYSTEM);
  }
}
 ...
 * on.
 */
void AddCommand( void(*CommandCallback)(char *Parameters),const char *CommandWord, char CmdGroup)
{
	CommandInfo	**CurrentCommand = &Commands;
...
void TabCompletion(char *CommandLine, int *location )
{
  ...
  if (CommandLine[0]!=REMOTE_CMD_CHAR)
	  ...
	  {
		  if((CurrentCommand->CommandGroup==CMD_SERVER) &&
         (0 == strncmp(&CommandLine[1], CurrentCommand->CommandWord, strlen(CommandLine)-1)))
		  {
...
void ListCommands()
{
  ...
  ConOut("---------------");
	while(CurrentCommand)
	{
		if (CurrentCommand->CommandGroup==CMD_LOCAL) ConOut("%s", CurrentCommand->CommandWord);
		CurrentCommand = CurrentCommand->NextCommand;
	}
  ...
  CurrentCommand = Commands;
	while(CurrentCommand)
	{
		if (CurrentCommand->CommandGroup==CMD_CLIENT) ConOut("%s", CurrentCommand->CommandWord);
		CurrentCommand = CurrentCommand->NextCommand;
	}
  ...
  CurrentCommand = Commands;
	while(CurrentCommand)
	{
		if (CurrentCommand->CommandGroup==CMD_SERVER) ConOut("%s", CurrentCommand->CommandWord);
		CurrentCommand = CurrentCommand->NextCommand;
	}
}
...
bool CommandExists(char* cmd_name)
{
	...
	while(CurrentCommand)
	{
    if (!strcmp(cmd_name, CurrentCommand->CommandWord)) return true;
    CurrentCommand = CurrentCommand->NextCommand;
	}
  return false;
}
