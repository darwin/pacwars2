...
void DrawCursor()
{
    ...
    if (strlen(CursorLine)<CHARS_PER_LINE) CursorLine[strlen(CursorLine)]=CursorChar;
    
    DrawText(CursorLine, ConsoleSurface, FontNumber, 4, ConsoleSurface->h-FontHeight(FontNumber));
}
/* ########################################################## */
...
void UpdateConsole()
{
    ...
    for( loop=0; loop<Screenline; loop++)
        DrawText(ConsoleLines[Screenline-loop+ConsoleScrollBack], ConsoleSurface, FontNumber, 4, loop*FontHeight(FontNumber));
    
    if( ConsoleScrollBack > 0 )
        for( loop=4; loop<OutputScreen->w-FontWidth(FontNumber)*2; loop+=FontWidth(FontNumber)*2)
            DrawText("^ ", ConsoleSurface, FontNumber, loop, ConsoleSurface->h-FontHeight(FontNumber));
        else 
            //DrawText(ConsoleLines[0], ConsoleSurface, FontNumber, 4, ConsoleSurface->h-FontHeight(FontNumber));
            /* ########################################################## */
            DrawCursor();
...
int InitConsole(const char *FontName, SDL_Surface *DisplayScreen, int lines)
{
    int	loop;
    SDL_Surface	*Temp;
    ...
    InputBackground = SDL_ConvertSurface(Temp, OutputScreen->format, SDL_SWSURFACE);
    SDL_FreeSurface(Temp);
...
int ConsoleBackground(const char *image, int x, int y)
{
    ...
    BackgroundImage = SDL_ConvertSurface(temp, OutputScreen->format, SDL_SWSURFACE);
    SDL_FreeSurface(temp);
    BackX = x;
    BackY = y;
