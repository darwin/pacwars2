		...
		if (msg_cpackets[n]->channel >= 0) {
			...
			case MSGID_WELCOME:
				if (net_client_status == NS_INITED) {
					...
					ConOut("Welcome to %s",
						   (char *) &MSG.buf[POS_MSG_WSNAME]);
					ConOut("Server version: %d.%02d", major, minor);
					ConOut("Server message: %s",
						   (char *) &MSG.buf[POS_MSG_WSMSG]);
					if (major > VERSION_MAJOR
						|| (major == VERSION_MAJOR
							&& minor > VERSION_MINOR)) {
						ConOut
							("Server version %d.%02d is greater than version of your program %d.%02d.\n");
						ConOut
							("Please download latest version of PacWars2. (%s)\n",
							 PW_WEBADDRESS);
					} else {
						net_client_status = NS_CONNECTED;
						server.status = SS_ACTIVE;
						smPlayVoice(SM_PREPARE, 100, 150);
						CL_ChangeMap((char *) &MSG.buf[POS_MSG_WMAPNAME],
									 (char *)
									 &MSG.buf[POS_MSG_WSCRIPTNAME]);
					}
				} else
					fprintf(stderr,
							"%d>Client: unexpected WELCOME message from %s",
							client_info.ticks,
							AddrToS(&msg_cpackets[n]->address));
				break;
			...
			case MSGID_SNAME:
				if (net_client_status == NS_CONNECTED) {
					memcpy(server.name, &MSG.buf[POS_MSG_SNAME],
						   LEN_MSG_SNAME);
...
int CL_Connect(char *server)
{
	...
	if (net_client_status != NS_UNINITED) {
		...
		if (msg_serverIP.host == INADDR_NONE) {
			...
			if (remote_host_name)
				ConOut("Connecting to %s [%s]", /*_strlwr(*/ remote_host_name	/*) */
					   , AddrToS(&msg_serverIP));
			else
			...
			memcpy(&msg_cpacket->data[POS_MSG_NAME], c_name.string,
				   PWP_MAX_CLIENT_NAME);
...
int CL_ChangeMap(char *mapname, char *scriptname)
{
	...
	if (MapLoaded == 0) {
		if (res != -10) {
			...
			ConOut("Client: script %s not found or not correct !",
				   scriptname);
