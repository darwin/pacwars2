...
{
  if ((strnum<0) || (strnum>=mapnumblockstr)) return SC_INDEX_OUT_OF_RANGE;
  
  BLKSTR *sb = (BLKSTR*) (server_info.game.map.mapblockstrpt + strnum*sizeof(BLKSTR));
...
{
  ABS(strnum);
  if ((strnum<0) || (strnum>=mapnumanimstr)) return SC_INDEX_OUT_OF_RANGE;
  
  ANISTR *sa = (ANISTR*) (server_info.game.map.mapanimstrendpt - strnum*sizeof(ANISTR));
...
{
  if (server_info.game.map.ChangeLayer(laynum)==-1) return SC_LAYER_UNDEFINED; else return SC_NOERR;
}

int smGetMapVal(int x, int y, int& val)
{
  if ((x<0) || (x>=mapwidth) || (y<0) || (y>=mapheight)) return SC_MAPTILE_OUT_OF_RANGE;
...
int smGetMapBlock(int x, int y, SCBLK& block)
{
  ...
  if (err) return err;
  if (blknum<0) return SC_TILE_IS_ANIMATED;
...
int smGetMapAnim(int x, int y, SCANI& anim)
{
  ...
  if (err) return err;
  if (aninum>=0) return SC_TILE_IS_STILL;
...
int smTrsMapBlockVal(int x, int y, int& val)
{
  ...
  { 
    myanpt = (ANISTR*) (server_info.game.map.mapanimstrendpt + *mymappt);
    val = ((BLKSTR*)(myanpt->ancuroff)) - (BLKSTR*)server_info.game.map.mapblockstrpt;
  }
  return SC_NOERR;
}
...
{
  if ((x<0) || (x>=mapwidth) || (y<0) || (y>=mapheight)) return SC_MAPTILE_OUT_OF_RANGE;
  if (((val>=0) && (val>=mapnumblockstr)) ||
      ((val<0) && (val<-mapnumanimstr))) return SC_INDEX_OUT_OF_RANGE;
...
int cmGetBlockStr(int strnum, SCBLK& block)
{
  ...
  BLKSTR *sb = (BLKSTR*) (client_info.game.map.mapblockstrpt + strnum*sizeof(BLKSTR));
...
int cmGetAnimStr(int strnum, SCANI& anim)
{
  ...
  ANISTR *sa = (ANISTR*) (client_info.game.map.mapanimstrendpt - strnum*sizeof(ANISTR));
...
{
  if (client_info.game.map.ChangeLayer(laynum)==-1) return SC_LAYER_UNDEFINED; else return SC_NOERR;
}
...
int cmTrsMapBlockVal(int x, int y, int& val)
{
  ...
  { 
    myanpt = (ANISTR*) (client_info.game.map.mapanimstrendpt + *mymappt);
    val = ((BLKSTR*)(myanpt->ancuroff)) - (BLKSTR*)client_info.game.map.mapblockstrpt;
  }
  return SC_NOERR;
}
